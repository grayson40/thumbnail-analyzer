...
// Fetch popular videos from YouTube by category
async function fetchPopularVideos(categoryId) {
  try {
    ...
    return response.data.items
      .filter(video => parseInt(video.statistics.viewCount) >= MIN_VIEW_COUNT)
      ...
      .map(video => ({
        ...
        ctr: calculateEstimatedCTR(video.statistics) // Estimated CTR based on available metrics
      }));
  } catch (error) {
